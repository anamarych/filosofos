...
bool tenedores[numFilosofos]; //array de tenedores por filosofo existente
Tmutex tenedor; //portero de asignacion
Tmutex pantalla; //impresiones en pantalla
...
void comer(void){
    ...
    while(!comiendo){
        mutex_lock(&tenedor);
        if(!tenedores[derecha] && !tenedores[izquierda]){
            tenedores[derecha] = true;
            tenedores[izquierda] = true;
            mutex_lock(&pantalla);
            printf("Filosofo %d: toma tenedores %d, %d\n", id,izquierda,derecha);
            mutex_unlock(&pantalla);
            comiendo = true;

            mutex_unlock(&tenedor);
            mutex_lock(&pantalla);
            ...
            mutex_lock(&pantalla);
            printf("Filosofo %d: libera tenedores %d, %d\n", id, izquierda,derecha);
            mutex_unlock(&pantalla);
        }
        mutex_unlock(&tenedor);     
    }
...
int main(void){
    ...
    mutex_init(&tenedor,1); //init tenedor
    
    for(indice = 0; indice < numFilosofos; indice++) {//init tenedores
        tenedores[indice] = false;
    }
